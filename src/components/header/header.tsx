...
export default function Header() {
  ...
  const [headerInfo, setHeaderInfo] = useState<UserInfo>(userInfo);
  ...
  const getData = async (token: string | null) => {
    ...
    const data = {
      ...
      generateCount: info.data.generateCount,
    };
  ...
  useEffect(() => {
    if (localStorage.getItem("access_token")) {
      ...
      getData(localStorageToken);
    }
  }, []);

  return (
    <div className="navbar bg-base-100 absolute top-0 z-[100]">
          ...
          >
            <div className="card-body">
              <span className="font-bold text-lg">
                이미지 생성 횟수: {headerInfo.generateCount}
              </span>
              <span className="text-info">포인트: {headerInfo.point}</span>
              <div className="card-actions">
                <button className="btn btn-primary btn-block">Buy Point</button>
              </div>
            </div>
          </div>
        </div>
